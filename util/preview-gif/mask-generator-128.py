...
for frame_idx in range(total_frames):
    ...
    for i in range(end_idx):
        ...
        # Fully revealed pixels are drawn opaque white (mask logic for downstream compositing)
    ...
    for t in range(trail_len):
        ...
        if 0 <= idx < len(spiral_indices):
            ...
            # Opacity fades from 75% → 0%
            alpha = int(192 * (1 - fade_fraction))
...
frames[0].save(
    output_path,
    save_all=True,
    append_images=frames[1:],  # rest of the animation
    duration=int(1000 / fps),  # frame duration in milliseconds
    loop=0,  # loop forever
    disposal=2  # restore to background before drawing next frame
)
