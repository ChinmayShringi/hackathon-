import { pgTable, integer, text, serial, varchar, boolean, timestamp, jsonb, unique, index, numeric } from "drizzle-orm/pg-core"
...
export const recipeOptionTagIcon = pgTable("recipe_option_tag_icon", {
...
export const users = pgTable("users", {
	...
	createdAt: timestamp("created_at", { mode: 'string' }).defaultNow(),
	updatedAt: timestamp("updated_at", { mode: 'string' }).defaultNow(),
	firstVisitAt: timestamp("first_visit_at", { mode: 'string' }).defaultNow(),
	handle: varchar(),
	...
	lastCreditRefresh: timestamp("last_credit_refresh", { mode: 'string' }).defaultNow(),
	accountType: integer("account_type"),
	accessRole: integer("access_role"),
}, (table) => [
...
// Component Registry System (align with canonical; timestamps as strings)
...
// User Media Library (migration-side schema; timestamps as strings)
