...
const DEFAULT_CONFIG: DeployConfig = {
  ...
  region: 'us-east-1',
  roleArn: 'arn:aws:iam::222845718616:role/VideoToGIF_Square-ExecutionRole'
};

async function createExecutionRole(awsEnv: any, region: string): Promise<string> {
  const roleName = 'VideoToGIF_Square-ExecutionRole';
...
async function deployToAWS(packageDir: string, config: DeployConfig) {
  ...
  const deployCommand = [
    ...
    '--memory-size 2048', // 2GB for FFmpeg processing
...
async function main() {
  console.log('🎬 VideoToGIF_Square Lambda Deployment');
  console.log('=====================================');
  ...
  console.log(`     --function-name ${config.functionName} \\`);
  console.log(`     --region ${config.region} \\`);
  console.log(`     --payload '{"remote_url":"https://example.com/video.mp4","bucket":"your-bucket","key":"thumbnails/video.gif","dimension":128}' \\`);
